...
namespace ns3 {
  uint32_t addressToIP(Address address);
  bool isAttacker(int ip);
  ...
  /*Converts from type Address to uint32_t*/
  uint32_t addressToIP(Address address)
  {
    uint8_t buf[4];
    address.CopyTo(buf);
  ...
  /*Converts from Human Readable Form i.e 192.168.2.1 to Decimal*/
  ...
  int indexFromIP (uint32_t ip, int type)
  {
    if(type == SERVER){
  ...
  void
    TcpSocketBase::ProcessSynSent (Ptr<Packet> packet, const TcpHeader& tcpHeader, const Address& fromAddress, const Address& toAddress)
    {
      static int synCount = 0;
      static int synackCount = 0;
      NS_LOG_FUNCTION (this << tcpHeader);
      printf("I am here\n");
      // Extract the flags. PSH and URG are not honoured.
      uint8_t tcpflags = tcpHeader.GetFlags () & ~(TcpHeader::PSH | TcpHeader::URG);
